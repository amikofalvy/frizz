...
try {
  ...
  if (argument("visual")) {
    for (const font of ["sans", "mono"]) {
      ...
      await until("browser version poll catches up", () => page.evaluate(() => document.querySelector("#update-restart-popover")?.textContent.includes("Server 0.13.1")), 15_000)
      await delay(250)
      ...
      assert.match(measurements.copy, /Server 0.13.1Launcher 0.13.0/)
