...
1. **Bump the version on `main`.** Raise `version` in the root [`package.json`](package.json) and commit it ALONE as `chore(release): X.Y.Z`. A patch bump is the convention, `feat(` commits included — match the last `chore(release):` and add one. Nothing else moves with it: no lockfile, no per-package manifest.
...
5. **Watch the run to the end, and do not call it cut until npm says so.** `gh run list --workflow=release.yml --limit 1` names the run; `gh run watch <id> --exit-status` follows it. The workflow typechecks, publishes through npm Trusted Publishing (OIDC — no token exists to rotate), tags `vX.Y.Z` on the released commit and creates the GitHub release. It asks the registry whether the version is already published rather than diffing commits, so a re-run or a `workflow_dispatch` can never double-publish. The release is cut when `npm view frizz version` prints the new version and `git ls-remote --tags origin vX.Y.Z` finds the tag — not when the push lands. If the run dies AFTER the publish (the tag or the GitHub release step), `gh workflow run release.yml --ref release` finishes the job: each step checks for its own artifact and skips what already exists.

**A version bump on `main` is INERT until `release` moves.** Pushing `main` publishes nothing, by design; that silence is why step 4 has two pushes. The daily runtime watch compares the root `package.json` against `npm view frizz version` and reports a bump that never shipped. The trap that has actually broken releases at the install step — `pnpm install --frozen-lockfile` disagreeing with a lockfile `nub` wrote — and the full reasoning live in [`plans/runtime-pin-bumps.md`](plans/runtime-pin-bumps.md) § Then cut a release.
